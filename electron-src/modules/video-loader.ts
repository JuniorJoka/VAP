...
class MuseLoaderModule extends Module {
  ...
  constructor() {
    super();
    this.seekFolders = [path.join(os.homedir(), 'Video')];
    this.supportedFormats = ['mp4'];
    this.videoList = [];
  }

  async load(): Promise<void> {
    ipcMain.handle(channel.video.READY, async () => {
  ...
  async getAllVideo(): Promise<void> {
      ...
      for (let fileDirectory of content) {
        ...
        if (isDirectory) {
          ...
          if (this.supportedFormats.includes(fileFormat)) {
            this.videoList.push(fileDirectory);
          }
        }
      }
  ...
  async getMuseMeta(): Promise<MuseMeta[]> {
    ...
    for (const track of this.videoList) {
  ...
  removeFolder(pathToFolder: string): void {
    ...
    if (pathInCollection) {
      this.seekFolders = this.seekFolders
        .slice(0, pathIndex)
        .concat(this.seekFolders.slice(pathIndex + 1));
    }
  }
